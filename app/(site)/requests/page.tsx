  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-ink-950">Quote requests</h1>
      ...
      {requests.length === 0 ? (
        <div className="card p-8 text-center">
          <p className="text-sm text-ink-600">No quote requests recorded yet.</p>
            ...
            return (
              <article key={r.id} className="card p-4">
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  <span className="mono text-sm font-semibold text-signal-600">{r.id}</span>
                  <span className="text-sm font-medium text-ink-900">{r.company}</span>
                  <span className="text-xs text-ink-500">{r.contact}</span>
                  <span className="ml-auto text-xs text-ink-400">
                    {new Date(r.createdAt).toLocaleString('en-GB')}
                  </span>
                </div>
                {r.requirement && (
                  <p className="mt-2 text-sm text-ink-600">{r.requirement}</p>
                )}
                {products.length > 0 && (
                  <ul className="mt-3 flex flex-wrap gap-1.5">
                    {products.map((p) => (
                      <li key={p.id}>
                        <Link
                          href={`/product/${p.id}`}
                          className="mono rounded border border-ink-200 px-2 py-0.5 text-[11px] text-ink-600 hover:border-signal-300 hover:text-signal-600"
                        >
                          {p.partNumber ?? p.id}
                        </Link>
                      </li>
