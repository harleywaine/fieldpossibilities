  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <header className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight text-ink-950 dark:text-white">Catalogue ingestion</h1>
        ...
        <>
                ...
                {Object.entries(validation.populated).map(([k, v]) => {
                  ...
                  return (
                    <div key={k} className="flex items-center gap-3">
                      <span className="w-44 shrink-0 text-xs text-ink-600 dark:text-ink-300">
                        {k.replace(/_/g, ' ')}
                      </span>
          ...
          {validation.warnings.length > 0 && (
            <section className="mb-8 rounded-xl border border-[color:var(--color-caution-600)]/25 bg-[color:var(--color-caution-600)]/8 p-5">
              <h2 className="text-[11px] font-semibold uppercase tracking-wider text-[color:var(--color-caution-600)] dark:text-[color:var(--color-caution-400)]">
                Data honesty notes
              </h2>
              <ul className="mt-2.5 space-y-1.5">
                {validation.warnings.map((w) => (
                  <li key={w} className="text-xs leading-relaxed text-ink-700 dark:text-ink-200">{w}</li>
                ))}
              </ul>
            </section>
          )}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="card p-4">
      <div className="text-xl font-semibold tabular-nums tracking-tight text-ink-900 dark:text-white">{value}</div>
      ...
      <div className="space-y-1">
        {items.map((i) => (
          <div key={i.name} className="flex items-center gap-2">
            <span className="w-40 shrink-0 truncate text-xs text-ink-600 dark:text-ink-300" title={i.name}>
              {i.name}
            </span>
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-ink-100 dark:bg-ink-800">
