...
export function AskPanel({ ids, query }: { ids: string[]; query: string }) {
  ...
  return (
    <section className="card p-5">
      <h2 className="text-sm font-semibold text-ink-900">Ask about these results</h2>
        ...
        <div className="mt-4 space-y-4">
          {thread.map((t, i) => (
            <div key={i} className="border-l-2 border-signal-500/30 pl-3">
              <p className="text-sm font-medium text-ink-900">{t.q}</p>
              <p className="mt-1.5 text-sm leading-relaxed text-ink-700">{t.a}</p>
      ...
      <div className="mt-4 flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-signal-400 focus:ring-4 focus:ring-signal-500/10"
        />
        <button
