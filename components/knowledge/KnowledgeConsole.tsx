      ...
      <div className="card p-5">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-[2px] border border-ink-200 bg-white px-3 py-2.5 text-sm outline-none transition focus:border-signal-500 focus:ring-4 focus:ring-signal-600/10"
          />
          <button
            onClick={() => ask(question)}
            disabled={busy || !question.trim()}
            className="shrink-0 rounded-[2px] bg-action-600 px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-action-500 disabled:opacity-40"
          >
            {busy ? 'Searching…' : 'Ask'}
          </button>
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {WORKFLOWS.map((w) => (
            <button
              ...
              onClick={() => ask(w.q)}
              disabled={busy}
              className="rounded-[2px] border border-ink-200 px-2.5 py-1 text-[11px] text-signal-600 transition-colors hover:border-signal-300 hover:bg-ink-50 disabled:opacity-40"
            >
              {w.label}
            </button>
          ))}
      ...
      {brief && (
        <div className="mt-6 space-y-6">
          ...
          <section>
            ...
            <div className="divide-y divide-ink-100 rounded-[2px] border border-ink-100 bg-white">
              {brief.fields.map((f) => (
                <div key={f.label} className="p-4">
                  <div className="flex flex-wrap items-baseline gap-2">
                    ...
                    {f.interpretation && (
                      <span className="rounded-[2px] bg-signal-600/10 px-1.5 py-0.5 text-[10px] font-semibold text-signal-600">
                        AI interpretation
                      </span>
                    )}
                  </div>
                  ...
                  {f.conflict && (
                    <div className="mt-2 rounded-[2px] border border-[color:var(--color-caution-600)]/30 bg-[color:var(--color-caution-600)]/8 p-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => ask(question, 'quote-brief')} disabled={busy}
              className="rounded-[2px] border border-ink-200 bg-white px-4 py-2 text-sm font-medium text-signal-600 transition-colors hover:bg-ink-50 disabled:opacity-40">
              Prepare quote brief
            </button>
            <button onClick={() => setPanel(brief.retrieval.documents.map(docToCitation))}
              className="rounded-[2px] border border-ink-200 bg-white px-4 py-2 text-sm font-medium text-signal-600 transition-colors hover:bg-ink-50">
    ...
    <section className="rounded-[2px] border border-signal-600/25 bg-signal-600/5 p-5">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className="ml-auto rounded-[2px] border border-ink-200 bg-white px-2.5 py-1 text-[11px] font-medium text-signal-600 hover:bg-ink-50"
        >
          {copied ? 'Copied' : 'Copy'}
        </button>
      </div>
      <pre className="mono max-h-[28rem] overflow-auto whitespace-pre-wrap rounded-[2px] bg-white p-4 text-xs leading-relaxed text-ink-800">
        {generated.text}
      </pre>
      {isResponse && (
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-ink-950/25" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-[2px] px-2 py-1 text-sm text-ink-400 hover:bg-ink-50">
            Close
          </button>
        </div>
        <ul className="space-y-3">
          {citations.map((c) => (
            <li key={c.documentId} className="rounded-[2px] border border-ink-100 p-3">
